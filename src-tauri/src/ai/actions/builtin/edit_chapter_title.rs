...
use crate::ai::prompts::{EditChapterTitleContext, PromptTemplates};
use crate::ai::types::{AiRequestData, Message, MessageRole};

/// 编辑章节标题输入参数
#[derive(Debug, Deserialize)]
pub struct EditChapterTitleInput {
    /// 小说 ID（必填）
    pub novel_id: Uuid,
...
/// AI 生成章节标题，或（有标题时）在原标题基础上优化
pub struct EditChapterTitleAction;

#[async_trait]
impl ActionHandler for EditChapterTitleAction {
    fn name(&self) -> &str {
        "edit_chapter_title"
    }
    ...
    async fn handle(&self, ctx: ActionContext) -> Result<ActionResponse, ActionError> {
        ...
        let title = call_ai(ctx, prompt).await?;
...
/// 反序列化入参
fn parse_input(ctx: &ActionContext) -> Result<EditChapterTitleInput, ActionError> {
...
fn render_prompt(
    ...
    let prompt_context = EditChapterTitleContext {
    ...
    templates
        .render_edit_chapter_title(&prompt_context)
