...
pub trait TemplateInstance {
    ...
    /// 强类型参数结构体
    type Params: DeserializeOwned + Validate + Serialize;
    ...
    /// 渲染：校验通过后，将 `Params` 序列化为 JSON 对象再交给 `TemplateManager`
    fn render(&self, manager: &TemplateManager, data: Value) -> Result<String, TemplateError> {
        let params = self.validate(&data)?;
        let ctx = serde_json::to_value(&params)
            .map_err(|e| TemplateError::InvalidContext(e.to_string()))?;
        manager.render(Self::TEMPLATE_ID, &ctx)
    }
}
